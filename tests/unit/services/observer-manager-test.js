...
module('Unit | Service | observer-manager', function(hooks) {
  ...
  hooks.beforeEach(function() {
    this.observerManager = this.owner.lookup('service:ember-scroll-modifiers@observer-manager');
  ...
  test('Creation', function(assert) {
    assert.ok(this.observerManager._admin, 'intersection-observer-admin is created');
  });

  test('observe', function(assert) {
    this.observerManager.observe('foo', this.options);
    assert.ok(this.observerManager._admin.observe.calledWith('foo', this.options), 'observe is called with correct parameters');
  });
  ...
  test('addEnterCallback', function(assert) {
    ...
    assert.ok(this.observerManager._admin.addEnterCallback.calledWith('foo', this.callback), 'addEnterCallback is called with correct parameters');
  });

  test('addExitCallback', function(assert) {
    this.observerManager.addExitCallback('foo', this.callback);
    assert.ok(this.observerManager._admin.addExitCallback.calledWith('foo', this.callback), 'addExitCallback is called with correct parameters');
  });
});
